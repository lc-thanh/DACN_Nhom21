...
export function AllBooksTable({
  ...
  const handlePickBook = (id: string) => {
    const book = bookPaginated?.items.find((book) => book.id === id);
    if (!book) return;

    const isBookAlreadyPicked = pickedBooks.some(
      (pickedBook) => pickedBook.book.id === book.id
    );
    if (isBookAlreadyPicked) {
      toast.info("Sách đã được chọn!");
      return;
    }
